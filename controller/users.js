...
function getUserDetailsController(req, reply) {
    ...
    if (userDetails) {
        ...
        reply.code(404).send({message: `The ID given ${id} is not found in records`});
    }
}
...
function createUserController(req, reply) {
    ...
    const updateUsersList = [...users, body];
    reply.code(201).send(updateUsersList);
}
...
function updateUserController(req, reply) {
    ...
    if (userDetails) {
        ...
        reply.code(404).send({ message: `The Username: ${username} is not found in records` })
    }
}
